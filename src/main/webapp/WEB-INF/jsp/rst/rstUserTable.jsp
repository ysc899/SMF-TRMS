...
<head>
	...
	<link rel="stylesheet" href="/css/rstUserTable.css?var=20210325_2">
	
	<title>결과관리::수진자별 결과관리(B)</title>
...
/* 			rstUserT += '	<tr onclick="javascript:resultClick(\''+resultList[i].DAT+'\', \''+ resultList[i].JNO +'\', \''+i+'\', this)">';
			...
			if(isNull(resultList[i].STS) || resultList[i].STS == " "){
				...
				if(resultList[i].STSD == "완료"){
					rstUserTA[a++] = '  <td style="max-width: 100px; color: #0000ff" >' + resultList[i].STSD + '</td>';
				}else{
					rstUserTA[a++] = '  <td style="max-width: 100px;" >' + resultList[i].STSD + '</td>';
				}
			}
			rstUserTA[a++] = '	</tr>'; 
		...
		for(i=0; i<rstUserDtl2.length; i++ ){
			...
			*/
    	    ...
    	    // 71259 종목인 경우, '이하' 문구가 없는 경우에는 빨강색 표시되어야 함. (단, 부속코드가 01,02 2개만 해당됨)
						...
						||(rstUserDtl2[i].O_CHR.indexOf("이하") <= -1 && rstUserDtl2[i].O_CHR.indexOf("미만") <= -1 && rstUserDtl2[i].O_GCD == "71259" && rstUserDtl2[i].O_ACD != "00")
	    	    		...
	    	    		||(rstUserDtl2[i].O_GCD == "72185" && rstUserDtl2[i].O_ACD == "01" && rstUserDtl2[i].O_CHR.indexOf("+") > -1)
					){
				
				rstResultTA[a++] = '  <td class="tool" style="max-width: 150px; font-weight: bold; color: #ff0000;">' + rstUserDtl2[i].O_CHR + '</td>';
				
			}else if(rstUserDtl2[i].O_GCD == "72185" && rstUserDtl2[i].O_ACD == "02" && rstUserDtl2[i].O_CHR.indexOf("+") > -1){
				rstResultTA[a++] = '  <td class="tool" style="max-width: 150px; font-weight: bold; color: #0000ff;">' + rstUserDtl2[i].O_CHR + '</td>';
			}else{
...
//			setTimeout("mastResult()",1000);
	...
	function searchValidation(){
		...
		//console.log("I_FDT : "+$("#I_FDT").val());
		//console.log("I_TDT : "+$("#I_TDT").val());
	...
	function mastResult(){
		...
		*/
		...
		if(photo_str == "Y"){
			...
			remark_str = "N";
			corona_ct_str = "N";	// 코로나 CT 결과 존재시, 결과 상세 화면에 'Corona CT' 문구를 출력할지 여부 flag(2020.03.05)
			cell_str = "N";			// 세포유전 결과 존재시, 결과 상세 화면에 'Corona CT' 문구를 출력할지 여부 flag(2022.08.29)
		}
										...
										<div class="main_srch_box">
											<div class="col-md-11-t col-md-10-t m-t-10">
												<div class="fleft label_margin fleft_next fmgbot" style="width : 40px;">
													<label for="" class="label_basic_t" >기간</label>
												</div>
												<div class="fleft fmgbot" style="width : 400px;">
												...
												<div class="fleft fmgtop" style="width : 160px;">
													<label for="I_NAM" class="label_basic_t">수진자명</label>
												...
												<div class="fleft fmgtop" style="width : 160px;">
													<label for="I_PCHN" class="label_basic_t">차트번호</label>
												...
												<div class="fleft fmgtop" id="DivHosSearch" style="display: none; ">
                                                    <label for="I_FNM" class="label_basic_t">병원</label>
												...
												<div class="fleft fmgtop"  style="width : 135px;">
													<button type="button" class="btn_srch_t" name = "btn_srch" onClick="javascript:search()">조회</button>
													<button type="button" class="btn btn-gray btn-sm" title="초기화" onClick="javascript:init2()">초기화</button>
												</div>
											</div>
											<div class="col-md-1-t col-md-2-t m-t-9">
											    <div class="fleft title_area_t open" style="float : right">
													<a href="#" class="btn_open_t">검색영역 접기</a>
												</div>
											</div>
										</div>
                                             ...
                                             <div class="srch_box_inner" >
                                                ...
                                                <div class="floatLeft2">
	                                               <div class="srch_item_v1">
	                                                   ...
	                                                   <input type="text" class="srch_put" id="I_PJKN" name="I_PJKN" placeholder="" maxlength="9" onkeydown="javascript:enterSearch(0)" style="width:61%;">
	                                               </div>
	                                               <div class="srch_item_v2">
