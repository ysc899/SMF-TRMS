...
<html lang="ko">
	...
	function setGrid1(){
	    ...
	    fields=[
	    	 {fieldName:"F100CHN"}	// 차트번호
	    	,{fieldName:"F100COR"}
	    	,{fieldName:"F100NAM"}	//수진자명
	    	,{fieldName:"F600ACD"}	//
	    ...
	    columns=[
	    	   {name:"F100CHN",		fieldName:"F100CHN",		header:{text:"차트번호"},				width:100 }
	    	  , {name:"F100NAM",		fieldName:"F100NAM",		header:{text:"수진자명",height:100},				width:120 }
	    ...
	    gridView.onItemChecked = function (grid, itemIndex, checked) {
			...
			for(var i in dataRows){
				...
				for(var  colNm in gridItem){
	 	    	...
	 	    	data["data"]= chartData;
	 	    	data["label"] = grid.getValue(gridIdx,"F100NAM").trim()+"-"+ grid.getValue(gridIdx,"SNM");
	 	    	graphData.push(data);
			}
			var gKind = $('#G_KIND').val().toLowerCase();
			graph("GraphChart", gKind, graphData);
	...
	function dataResult(){
		...
		var formData = $("#searchForm").serializeArray();
		ajaxCall("/statsPatItemList.do", formData);
	}
...
// 			iframe.gridViewRead(gridValus);
	...
	function pageCNP(type){
		...
		if(type == "P"){ 			// 이전결과 페이지
			...
			$('#I_NAMP').val(dataProvider.getValue(0, "F100NAM"));
			$('#I_CHN').val(dataProvider.getValue(0, "F100CHN"));
			$('#I_ICNT').val(page);
		}
		if(type == "N"){ 			// 다음 결과 페이지
			count++;
			...
			$('#I_NAMP').val(dataProvider.getValue((page-1), "F100NAM"));
			$('#I_CHN').val(dataProvider.getValue((page-1), "F100CHN"));
			$('#I_ICNT').val(page+1);
		}
		if(count == 0){
			$('#I_PNN_N').removeClass("disabled");
				 ...
				 <div class="srch_box">
					<div class="srch_box_inner">
						...
						<div class="srch_item">
							...
							<input type="text" class="srch_put" id="I_NAM" name="I_NAM" value=""  onkeydown="return enterSearch(event)" >
						</div>              
					</div>
