...
/*
 엑셀
 */
public class DownloadUtoBio1 extends ResultDownload {
	boolean debug = false;
	...
	public DownloadUtoBio1(String id, String fdat, String tdat, Boolean isRewrite) {
		setId(id);
		setfDat(fdat);
		settDat(tdat);
	 ...
	 public void makeDownloadFile()
	  {
	    ...
	    try
	    {
	      this.workbook = Workbook.createWorkbook(new File(this.savedir + makeOutFile()));
	      this.wbresult = this.workbook.createSheet("Result", 0);
	      String[] ArraryResult = null;
	      ArraryResult = new String[] { "병원코드", "수탁ID", "접수일", 
	        "차트번호", "이름", "성별", "생년월일", "검사코드", "한글명", 
	        "영문명", "검체ID", "검체명", "바코드", "수탁일시", "검사결과", 
	        "서술결과", "리마크", "판정", "참고치", "수탁" };
	      for (int i = 0; i < ArraryResult.length; ++i) {
	        this.label = new jxl.write.Label(i, 0, ArraryResult[i]);
	        this.wbresult.addCell(this.label);
	      }
	    }
...
//				System.out.println(patName[i]+","+inspectCode[i] );
			...
			for (int i = 0; i < cnt; i++) {
		        ...
		        if ((hosCode[i].trim().equals("30897") || hosCode[i].trim().equals("31285")) && 
		                (data[14].length() > 6))
		              {
		                data[14] = data[14].substring(0, 6);
		              }
...
//				System.out.println(rcvDate[i] + "^^^" + rcvNo[i]);//양태용 찾았음 여기서 볼것!!!
				...
				if ( (hosCode[i].trim().equals("30286") || hosCode[i].trim().equals("30310"))
						&& (inspectCode[i].trim().substring(0, 5).equals("00095") || inspectCode[i].trim().substring(0, 5).equals("31010")
								...
								|| inspectCode[i].trim().equals("71139")|| inspectCode[i].trim().equals("52001")|| inspectCode[i].trim().equals("52002")
								|| inspectCode[i].trim().equals("52003")|| inspectCode[i].trim().equals("52004")|| inspectCode[i].trim().equals("52005")
								|| inspectCode[i].trim().equals("52006")|| inspectCode[i].trim().equals("52007")|| inspectCode[i].trim().equals("52008")
								|| inspectCode[i].trim().equals("52009")
...
//					System.out.println("==========================");
					...
					if (!isTxtRltB && (inspectCode[i].trim().substring(0, 5).equals("21653")|| inspectCode[i].trim().substring(0, 5).equals("64631") || inspectCode[i].trim().substring(0, 5).equals("00655"))&& hosCode[i].trim().equals("16702")) {
						data[34] = "별지보고"; // 단문결과
					}
...
//						}
					...
					if (!isTxtRltB
							&& (hosCode[i].trim().equals("12657") && inspectCode[i].trim().length() == 7 && (inspectCode[i].trim().substring(0, 5)
									.equals("00217")))) { // 단독
					...
					if (!isTxtRltB
							&& (hosCode[i].trim().equals("29434") && (inspectCode[i]
									.trim().substring(0, 5).equals("00095")|| inspectCode[i].trim().substring(0, 5)
									.equals("00083")))) { // 단독
						data[35] = appendBlanks("검  사  명 ", 30) + "\t" + appendBlanks("결    과", 21) + " \t" + "참    고    치";
						data[35] += getDivide() + "\r\n"
					...
					if (!isTxtRltB
							&& (hosCode[i].trim().equals("29598") && inspectCode[i].trim().length() == 7 && (inspectCode[i].trim().substring(0, 5)
									.equals("00095") ))) { // 단독
			     ...
			     if (!isTxtRltB
					...
					if(inspectCode[i].trim().substring(0, 5).equals("80014"))
					{
//						System.out.println("PBS 결과 테스트");
					...
					if (!isTxtRltB && hosCode[i].trim().equals("29659") && inspectCode[i].trim().length() == 7
							&& (inspectCode[i].trim().substring(0, 5).equals("71252")//20160617 문장요청
									...
									||inspectCode[i].trim().substring(0, 5).equals("41246")
					...
					if (!isTxtRltB && hosCode[i].trim().equals("29659") 
							&& (inspectCode[i].trim().substring(0, 5).equals("21380")
									...
									||inspectCode[i].trim().substring(0, 5).equals("71011"))) { // 한마음의료재단 제일병원(진단) 21380 검사 리마크 따로 저장
					...
					if (!isTxtRltB && inspectCode[i].trim().length() == 7 && (hosCode[i].trim().equals("29516") 
							|| hosCode[i].trim().equals("30300") || hosCode[i].trim().equals("28521"))
							&& (inspectCode[i].trim().substring(0, 5).equals("11101"))) { // 단독
...
//						System.out.println(data[39]);
					...
					if (!isTxtRltB && (isMAST_Two(inspectCode[i].trim().substring(0, 5)) || isMAST(inspectCode[i].trim().substring(0, 5)))&&//
					   (!hosCode[i].trim().equals("21954")	&& !hosCode[i].trim().equals("22015") && !hosCode[i].trim().equals("12640") && !hosCode[i].trim().equals("30355"))) { 
						try {// 삼육제외
...
//						System.out.println("2");
				...
				if ((hosCode[i].trim().equals("30286") || hosCode[i].trim().equals("30310")) && (inspectCode[i].trim().equals("21274"))) {
					{
						data[34]="";
					}
				...
				if(hosCode[i].trim().equals("12657") 
						&& (inspectCode[i].trim().equals("81399") || inspectCode[i].trim().equals("71325") || inspectCode[i].trim().equals("70017"))){
					data[36] = "별지 보고";
				}
				...
				//세종국민건강검진의원(검진)
		          if (hosCode[i].trim().equals("30897")) {
		              data[19] = "30897";
		              data[9] = "1F64AA24-04FD-4F84-BDC9-5719CA046A0F";
		            }
				
		          
		          
				// !
					//동탄시티병원(검진) 20171122 양태용
		          if (hosCode[i].trim().equals("31285")) {
		              data[19] = "31285";
		              data[9] = "1F64AA24-04FD-4F84-BDC9-5719CA046A0F";
		            }
...
//		System.out.println("test");
	...
	private boolean is24580_24080(String[] hosCode, int i) {
		return hosCode[i].trim().equals("24580") || hosCode[i].trim().equals("24080")
				...
				|| hosCode[i].trim().equals("27993") || hosCode[i].trim().equals("28272")
