...
public class DownloadHAMC extends ResultDownload {
	...
	public DownloadHAMC(String id, String fdat, String tdat, Boolean isRewrite) {
		isDebug = false;
		setId(id);
		setfDat(fdat);
		settDat(tdat);
...
//				String[] _tmp2 = new String[4];
				...
				data[1] = patName[i]; // 수진자명
				data[2] = specNo[i].trim(); // 검체번호
				data[3] = inspectCode[i].trim(); // 병원검사코드
				data[6] = rcvDate[i] + "-" + rcvNo[i] + "-"
						+ inspectCode[i].trim(); // 접수일자,접수번호,검사코드
				data[7] = inspectName[i].trim(); // 검사명
				...
				if (resultType[i].trim().equals("C")) {
					System.out.println(data[6]);
					data[8] = getUnit(unit[i]).trim(); // 참고치단위
					data[9] = getReference(
							new String[] { inspectCode[i], lang[i], history[i],
									sex[i] }).trim(); // 참고치
					data[4] = result[i].trim(); // 검사결과
				} else {
					data[5] = getTextResultValue(hosCode[i], rcvDate[i],
							rcvNo[i], inspectCode[i]).trim(); // 문장결과
				}
...
//					o_writer = new OutputStreamWriter(f_outStream);				// 인코딩 수정전
				...
				text.append(data[0] + deli + data[1] + deli + data[2] + deli
						...
						+ deli + data[6] + deli + data[7] + deli + data[8]
						+ deli + data[9] + deli + data[10] + deli + "\r\n");
