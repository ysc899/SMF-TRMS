...
public class DownloadJejuKorea extends ResultDownload {
	...
	public DownloadJejuKorea() {
		// isDebug = false;
	}

	public DownloadJejuKorea(String id, String fdat, String tdat, Boolean isRewrite) {
		setId(id);
		setfDat(fdat);
		settDat(tdat);
...
//					o_writer = new OutputStreamWriter(f_outStream);            // 인코딩 수정전
				...
				// 차트번호 . 이름 , 접수일 ,접수번호 , 바코드 번호, 처방코드 , 결과
				text.append(data[0]+ deli + data[4]+ deli + data[5]+ deli + data[6] + deli + data[1] + deli + data[2] + deli + data[3] + "\r\n");
