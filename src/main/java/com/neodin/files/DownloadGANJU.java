...
public class DownloadGANJU extends ResultDownload {
	boolean isDebug = false;

	boolean isData = false;

	public DownloadGANJU() {
		isDebug = false;
	}

	public DownloadGANJU(String id, String fdat, String tdat, Boolean isRewrite) {
		isDebug = false;
		setId(id);
		setfDat(fdat);
		settDat(tdat);
		...
		try {
			...
			for (int i = 0; i < cnt; i++) {
				...
				if (resultType[i].trim().equals("C")) {
					if (inspectCode[i].trim().length() == 7 && ((hosCode[i].toString().trim().equals("15710") ||hosCode[i].toString().trim().equals("26070")) && (inspectCode[i].trim().substring(0, 5).equals( "71251")
				...
				text.append(data[2] + deli + data[0] + deli + data[1] + deli +  data[3] + deli + result_str+ deli +reference_str+ "\r\n");
