...
/*
 엑셀
 */
public class DownloadCERAN extends ResultDownload {
	boolean debug = false;
	public DownloadCERAN() {
		initialize();
	}
	...
	public DownloadCERAN(String id, String fdat, String tdat, Boolean isRewrite) {
		setId(id);
		setfDat(fdat);
		settDat(tdat);
		...
		try {
			...
			for (int i = 0; i < cnt; i++) {
				...
				if (resultType[i].trim().equals("C")) {
					...
					if (!isTxtRltC && (inspectCode[i].trim().equals("31100")||inspectCode[i].trim().equals("31101")
