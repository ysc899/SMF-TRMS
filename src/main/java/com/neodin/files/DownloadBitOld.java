 ...
 *
 */
public class DownloadBitOld extends ResultDownload {
	boolean isDebug = false;

	boolean isData = false;

	public DownloadBitOld() {
		isDebug = false;
	}

	public DownloadBitOld(String id, String fdat, String tdat, Boolean isRewrite) {
		isDebug = false;
		setId(id);
		setfDat(fdat);
		settDat(tdat);
		...
		try {
			...
			for (int i = 0; i < cnt; i++) {
				...
				if (hosCode[i].toString().trim().equals("26070") &&
						(inspectCode[i].trim().substring(0, 5).equals( "31014")|| inspectCode[i].trim().substring(0, 5).equals( "31019") ||inspectCode[i].trim().substring(0, 5).equals( "31077"))
						&&"중　간　결　과　보　고".equals(data[4])
						) {
					continue;
				}
//				//20190219 양태용 31154 중간보고 집계제외
...
//									|| inspectCode[i].trim().substring(0, 5).equals( "71252")|| inspectCode[i].trim().substring(0, 5).equals( "81148")
...
//				if (hosCode[i].toString().trim().equals("31154")
//						&& ( inspectCode[i].trim().substring(0, 5).equals( "11052") || inspectCode[i].trim().substring(0, 5).equals( "72185")
//								|| inspectCode[i].trim().substring(0, 5).equals( "00690") || inspectCode[i].trim().substring(0, 5).equals( "00691")
//								|| inspectCode[i].trim().substring(0, 5).equals( "31001") || inspectCode[i].trim().substring(0, 5).equals( "81494")
//								|| inspectCode[i].trim().substring(0, 5).equals( "05495")
//						)) {
//					data[4] = "별지보고";
//					}
//				
...
//				if (hosCode[i].toString().trim().equals("31154")) {
//					data[4] = data[4].replaceAll("                      최　종　결　과　보　고", "최　종　결　과　보　고");
//					}
//					
				...
				// 접수일시$ 차트번호$ 환자이름$ 접수번호$ 검체코드(바코드번호)$ 검사코드$ 결과  $H/L판정치$참고치
...
//				text.append(data[0] + deli + data[1] + deli + cnsDate + deli +  rcvNo[i] + deli + data[2] + deli + data[3] + deli + result_str+ deli +data[11] + deli +reference_str+ "\r\n");
				text.append(cnsDate + deli + data[0] + deli + data[1] + deli + data[2] + deli + data[3] + deli + result_str + "\r\n"); //+ deli +data[11] + deli + reference_str + "\r\n");
