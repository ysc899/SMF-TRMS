...
public class DownloadEumcExcel extends ResultDownload {
...
//	protected String gubun3 = "Reported by 씨젠의료재단 한송희 M.D\r\nReviewed by 이대목동병원 소민경/박설희 M.D";
	...
	public DownloadEumcExcel() {
		// isDebug = false;
	}

	public DownloadEumcExcel(String id, String fdat, String tdat, Boolean isRewrite) {
		setId(id);
		setfDat(fdat);
		settDat(tdat);
...
//	}
	...
	public String getResultTriple(java.lang.String[] id) {
		...
		boolean is99933 = false; // Integrated test 1차
...
//		boolean is90029 = false; // Double marker
		...
		String temp_title = "<< 기형아검사 검사결과보고 >>" ;
...
//														rtnResult += "\r\n\r\n* 다운증후군/에드워드 증후군 : \r\n\r\n";
	...
	//		rtnResult += "\r\n\r\n* Comment : \r\n\r\n";
				...
				rtnResult += "\r\n" + appendBlanks(" Down SD Risk by Age 만 35세 ", 31)
						//					+ appendBlanks(" 만 35세 ", 12)
						//					+ appendBlanks("" .equals(Common.cutZero (tempArray [15]).trim()) ?"": "1 : " +Common.cutZero(tempArray[15]), 13)
											+ appendBlanks("" , 21)
											+ appendBlanks(tempArray[14], 10);
...
//		if (id[2].toString().equals("41381")) {	
	...
	//		System.out.println(tempArray[23]+"test");
...
//		}
	...
	public boolean isTriple(String gcd) {
		...
		if (gcd.equals("41383"))
			return true;
		if (gcd.equals("41338"))
			return true; //20191007 단문으로 처리 요청 했다가 20191028 다시 장문으로 처리 요청 해당 코드 트리플검사로서 묶는 검사였으나 개별로도 발생해 단독으로 생성시 문제 발생함 나현정과장 요청
		if (gcd.equals("41121"))
			return true;
		if (gcd.equals("41359"))
			return true;
...
//					ArraryResult = (new String[] {"챠트번호", "검체번호","고정번호" ,"처방코드","결과"});
					
					for (int i = 0; i < ArraryResult.length; i++) {
//						label = new jxl.write.Label(i, 1, ArraryResult[i] );
//						wbresult.addCell(label);
						
//						  label = new jxl.write.Label(0, i, "데이터.."+i); 
					      wbresult.addCell(label); 
...
//				}
				...
				if (inspectCode[i].trim().length() == 7 && (hosCode[i].trim().equals("15532") || hosCode[i].toString().equals("32473")) 
						&& ( inspectCode[i].trim().substring(0, 5).equals("05567") && !inspectCode[i].trim().equals("0556756"))) { // 단독
					continue ;
				} 
				...
				if (resultType[i].trim().equals("C")) {
					...
					try {
						if (isTriple(inspectCode[i])) {
							data[5] = getResultTriple(new String[] {rcvDate[i], rcvNo[i], inspectCode[i] });
							...
							(!inspectCode[i].trim().substring(0, 5).equals("11101") && !inspectCode[i].trim().substring(0, 5).equals("11301"))) {
								...
								(inspectCode[i].trim().substring(0, 5).equals("81617")|| inspectCode[i].trim().substring(0, 5).equals("05028")
										|| inspectCode[i].trim().substring(0, 5).equals("05063")|| inspectCode[i].trim().substring(0, 5).equals("05066"))) {
									data[5] = appendBlanks("검  사  명 ", 30)  + "        " + appendBlanks("결    과", 21) + "        "  + "참    고    치";
									data[5] += getDivide() + "\r\n" + appendBlanks(inspectName[i], 30) + "        " + appendBlanks(result[i], 21) 
												+ "        " + getReferenceValue(new String[] {inspectCode[i], lang[i], history[i], sex[i]});
									curDate = rcvDate[i];
									curNo = rcvNo[i];
									for ( thisTimeCode = inspectCode[i++].trim().substring(0, 5); thisTimeCode.equals(inspectCode[i].trim().substring(0, 5)) 
									...
									if (inspectCode[i].trim().length() == 7 && hosCode[i].toString().equals("32473") && 
									(inspectCode[i].trim().substring(0, 5).equals("81617"))) {
										data[5] = appendBlanks("검  사  명 ", 30)  + "        " + appendBlanks("결    과", 21) + "        "  + "참    고    치";
										data[5] += getDivide() + "\r\n" + appendBlanks(inspectName[i], 30) + "        " + appendBlanks(result[i], 21) 
													+ "        " + getReferenceValue(new String[] {inspectCode[i], lang[i], history[i], sex[i]});
										curDate = rcvDate[i];
								...
								if (inspectCode[i].trim().length() == 7 && (hosCode[i].toString().equals("15532")|| hosCode[i].toString().equals("32473")) && 
									(inspectCode[i].trim().substring(0, 5).equals("00309")
										)) {
									data[5] = "** " + inspectName[i] +" **\r\n\r\n";
									data[5] += appendBlanks("Fraction ", 30) + "        " + appendBlanks("%(g/dL)", 21) + "        " + "Reference Range\r\n" ;
									data[5] +="----------------------------------------------------------------------------------------";
							//		data[5] += getDivide() + "\r\n" + appendBlanks(inspectName[i], 30) + "        " + appendBlanks(result[i], 21) 
							//					+ "        " + getReferenceValue(new String[] {inspectCode[i], lang[i], history[i], sex[i]}).trim();
									...
									for ( thisTimeCode = inspectCode[i++].trim().substring(0, 5); thisTimeCode.equals(inspectCode[i].trim().substring(0, 5)) 
										 ...
										 data[5] += "\r\n" + appendBlanks(inspectName[i], 30) + "\t" + appendBlanks(result[i], 21) + "\t" 
											 + getReferenceValue(new String[] {inspectCode[i], lang[i], history[i], sex[i]}).trim();{
										if (inspectCode[i].trim().substring(5, 7).equals("06")){
											data[5] +="\r\n----------------------------------------------------------------------------------------";
										}}
										if (++i == cnt)
											break;
									...
									if (inspectCode[i].trim().length() == 7 && (hosCode[i].toString().equals("15532")|| hosCode[i].toString().equals("32473")) && 
										(inspectCode[i].trim().substring(0, 5).equals("00323") ||inspectCode[i].trim().substring(0, 5).equals("00334")
											)) {
										data[5] = "** " + inspectName[i] +" **\r\n\r\n";
										data[5] += appendBlanks("Fraction ", 30) + "        " + appendBlanks("%(mg/dL)", 21) + "        " + "Reference Range\r\n" ;
										data[5] +="----------------------------------------------------------------------------------------";
								//		data[5] += getDivide() + "\r\n" + appendBlanks(inspectName[i], 30) + "        " + appendBlanks(result[i], 21) 
								//					+ "        " + getReferenceValue(new String[] {inspectCode[i], lang[i], history[i], sex[i]}).trim();
										curDate = rcvDate[i];
									...
									if (inspectCode[i].trim().length() == 7 && ( hosCode[i].toString().equals("15532") || hosCode[i].toString().equals("32473") )&& 
									(	inspectCode[i].trim().substring(0, 5).equals("00317") || inspectCode[i].trim().substring(0, 5).equals("00318")
											|| inspectCode[i].trim().substring(0, 5).equals("00333") || inspectCode[i].trim().substring(0, 5).equals("00334")
										...
										if (inspectCode[i].trim().length() == 7 && ( hosCode[i].toString().equals("15532") || hosCode[i].toString().equals("32473"))&& 
										( inspectCode[i].trim().substring(0, 7).equals("0556756")
												)) {
											data[5] = "** " + inspectName[i] +" ** : "  + result[i] + " \r\n\r\n "
												+ getReamrkValue99(hosCode[i], rcvDate[i], rcvNo[i], rmkCode[i])+"\r\n";
										}
							...
							&& (inspectCode[i].trim().substring(0, 5).equals("72059")||inspectCode[i].trim().substring(0, 5).equals("72018")
									||inspectCode[i].trim().substring(0, 5).equals("81807")	
								...
								if (inspectCode[i].trim().length() == 7 && hosCode[i].toString().equals("24874")) {
									data[5] = appendBlanks("검  사  명 ", 30) + "        " + appendBlanks("결    과", 21) + "        " + "참    고    치";
									if (!inspectCode[i].trim().equals("0040500")) {
										data[5] += "\r\n" + appendBlanks(inspectName[i], 30) + "        " + appendBlanks(result[i], 21) + "        " + data[7];
									}
									data[5] = "";
									curDate = rcvDate[i];
									...
									for ( thisTimeCode = inspectCode[i++].trim().substring(0, 5); thisTimeCode.equals(inspectCode[i].trim().substring(0, 5)) && curDate.equals(rcvDate[i].trim()) && curNo.equals(rcvNo[i].trim());) {
										//!
										//if (inspectCode[i].trim().substring(0, 5).equals("00405")) {
										//System.out.println("");
										//}
										if (!inspectCode[i].trim().equals("0040500")) {
											data[5] += "\r\n" + appendBlanks(inspectName[i], 30) + "        " + appendBlanks(result[i], 21) + "        " + getReferenceValue(new String[] {inspectCode[i], lang[i], history[i], sex[i]}).trim();
					...
					if (hosCode[i].toString().equals("24874")||hosCode[i].toString().equals("30081")) {   
						if (inspectCode[i].trim().equals("31010") || inspectCode[i].trim().equals("31016") || inspectCode[i].trim().equals("31019") || inspectCode[i].trim().equals("31014") || inspectCode[i].trim().equals("31059")) {
							data[5] = '$' + getTextResultValue(hosCode[i], rcvDate[i], rcvNo[i], inspectCode[i]); //문장결과
						} else {
							data[5] = getTextResultValue(hosCode[i], rcvDate[i], rcvNo[i], inspectCode[i]); //문장결과
						}
					} 
				...
				if (inspectCode[i].trim().equals("31100")||inspectCode[i].trim().equals("31101")
					...
					data[5] = getTextResultValue(hosCode[i], rcvDate[i], rcvNo[i], inspectCode[i]);
			//		data[8] = data[5].trim();
					try {
			...
			//				data[8] = data[5].trim();
			//				i++;				20191031 양태용 해당 코드가 아닌 다른 코드가 결과 덮어씌어 버그발생함 (프로그램버그)
							// culture_flag = true;
						} else {
							data[5] = getTextResultValue(hosCode[i], rcvDate[i], rcvNo[i], inspectCode[i]);
			...
			//			data[8] = data[5].trim();
				...
				//20190710 센시코드가 앞일 경우 위와 반대 상황
				
				if (inspectCode[i].trim().equals("81399")) {
					
					data[5] = getTextResultValue(hosCode[i], rcvDate[i], rcvNo[i], inspectCode[i]);
			//		data[8] = data[5].trim();
					try {
						if (inspectCode[i - 1].substring(0, 5).equals("31080")
								&& rcvNo[i].equals(rcvNo[i - 1])
								&& rcvDate[i].equals(rcvDate[i - 1])) {
							data[5] =  getTextResultValue(hosCode[i], rcvDate[i], rcvNo[i], inspectCode[i - 1]) + data[5] + "\r\n" ;
			//				data[8] = data[5].trim();
			//				i++;  						20191031 양태용 해당 코드가 아닌 다른 코드가 결과 덮어씌어 버그발생함 (프로그램버그)
							// culture_flag = true;
						} else {
			...
			//			data[8] = data[5].trim();
				...
				if (inspectCode[i].trim().substring(0, 5).equals("71280")) {
					if(data[5].equals("Less than detection limit")){
						data[5] = "Not detected";
					}
				}
				if (inspectCode[i].trim().substring(0, 5).equals("31031")) {
					data[5] = replaceCrLf(result[i]);
				}
