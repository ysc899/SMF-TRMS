...
public class DownloadDangGin extends ResultDownload {
	boolean isDebug = false;

	boolean isData = true;

	public DownloadDangGin() {
		// isDebug = false;
		initialize();
	}
	...
	public DownloadDangGin(String id, String fdat, String tdat,
			Boolean isRewrite) {
		setId(id);
		setfDat(fdat);
		settDat(tdat);
...
//			}
			...
			for (int i = 0; i < cnt; i++) {
				...
				if (resultType[i].trim().equals("C")) {
					...
					// !
					if ((hosCode[i].trim().equals("24619")||hosCode[i].trim().equals("27468"))&& inspectCode[i].trim().length() == 7 
					...
					else if (hosCode[i].trim().equals("23379")&& inspectCode[i].trim().length() == 7 
									...
									|| inspectCode[i].trim().substring(0, 5).equals("71252")|| inspectCode[i].trim().substring(0, 5).equals("71259"))) 
				{
					...
					} else if (hosCode[i].trim().equals("28336")&& (inspectCode[i].trim().substring(0, 5).equals("72185")||inspectCode[i].trim().substring(0, 5).equals("31083")))
					{
						...
						i--;
					} else if (hosCode[i].trim().equals("27466")&&(inspectCode[i].trim().substring(0, 5).equals("72185")))
					{
						data[12] = appendBlanks("검  사  명 ", 30) + "\t"+ appendBlanks("결    과", 21) + " \t"+ "참    고    치";
						data[12] += "\r\n" + appendBlanks(inspectName[i], 30)+ "\t" + appendBlanks(result[i], 21) + "\t"+ data[15];
						data[11] = "";
					...
					try {
						remarkCode = rmkCode[i].trim();//20160811 요기로 바꿈
							...
							//remarkCode = rmkCode[i].trim(); 20160811 리마크 수정 작업 양태용이 지켜봤음
								if(hosCode[i].trim().equals("10747")){
