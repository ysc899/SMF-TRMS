 ...
 */
public class DownloadYOUNCT extends ResultDownload {
	boolean debug = false;
	public DownloadYOUNCT() {
		initialize();
	}
	...
	public DownloadYOUNCT(String id, String fdat, String tdat, Boolean isRewrite) {
		setId(id);
		setfDat(fdat);
		settDat(tdat);
	...
	public void makeDownloadFile() {
			...
			try {
				...
				ArraryResult = new String[] { "검사ID", "접수ID", "접수일자", "검체번호",
						"챠트번호", "환자이름", "주민번호", "유형", "코드", "명칭", "의뢰일자",
						"검사결과", "서술결과", "하이로우", "단위", "참고치"};
	...
	public void processingData(int cnt) {
		try {
			...
			for (int i = 0; i < cnt; i++) {
				...
				if (hosCode[i].trim().equals("28107")||hosCode[i].trim().equals("28551")||hosCode[i].trim().equals("28790")) {
					try {
						data[18] = getPatientInformationNew(rcvDate[i],rcvNo[i]).toString(); // 병동
					} catch (Exception eee) {
						data[18] = ""; // 처방일자
				...
				// 새부산병원 0009501 외에 삭제 처리 
				if (hosCode[i].trim().equals("28830") && (inspectCode[i].trim().substring(0, 5).equals("00095")  && ! inspectCode[i].trim().equals("0009501"))) { // 단독
					continue;
				}
				...
				if (resultType[i].trim().equals("C")) {
					...
					if (!isTxtRltB && inspectCode[i].trim().length() == 7 && (hosCode[i].trim().equals("28668") ||hosCode[i].trim().equals("15710") )&& (inspectCode[i].trim().substring(0, 5).equals("11002")
...
//								data[35] += getDivide() + "\r\n" + appendBlanks(inspectName[i], 30) + "\t" + appendBlanks(result[i], 21) + "\t"	+ getReferenceValue(new String[] { inspectCode[i], lang[i], history[i], sex[i], age[i] }).trim();
					...
					if (!isTxtRltB && (hosCode[i].trim().equals("26609")||hosCode[i].trim().equals("25663")) && (inspectCode[i].trim().substring(0, 5).equals("31001")//
							||inspectCode[i].trim().substring(0, 5).equals("00673")
...
//								data[35] += getDivide() + "\r\n" + appendBlanks(inspectName[i], 30) + "\t" + appendBlanks(result[i], 21) + "\t"	+ getReferenceValue(new String[] { inspectCode[i], lang[i], history[i], sex[i], age[i] }).trim();
					...
					if (!isTxtRltB && hosCode[i].trim().equals("23401") && inspectCode[i].trim().length() == 7
						...
						// !
					}	if (!isTxtRltB && hosCode[i].trim().equals("28858") && inspectCode[i].trim().length() == 7
							&& (inspectCode[i].trim().substring(0, 5).equals("00822") || inspectCode[i].trim().substring(0, 5).equals("00802") 
									|| inspectCode[i].trim().substring(0, 5).equals("11002") || inspectCode[i].trim().substring(0, 5).equals("00806") || inspectCode[i].trim().substring(0, 5).equals("00095") )) {
						data[35] = appendBlanks("검  사  명 ", 30) + "\t" + appendBlanks("결    과", 21) + " \t" + "참    고    치";
						data[35] += getDivide() + "\r\n" + appendBlanks(inspectName[i], 30) + "\t" + appendBlanks(result[i], 21) + "\t"
								+ getReferenceValue(remark);
						data[34] = ""; // 문자결과
					...
					if (!isTxtRltB
							...
							&& ( inspectCode[i].trim().substring(0, 5).equals("31001")//inspectCode[i].trim().substring(0, 5).equals("00405") ||
									...
									|| inspectCode[i].trim().substring(0, 5).equals("21061"))) { // 단독아님
						data[35] = appendBlanks("검  사  명 ", 30) + appendBlanks("결    과", 21) + "참    고    치";
					...
					if (!isTxtRltB
							&& ( ( hosCode[i].trim().equals("28982") ) && (inspectCode[i].trim().substring(0, 5).equals("72194")||inspectCode[i].trim().substring(0, 5).equals("72199")))) { // 단독
						data[35] = appendBlanks("검  사  명 ", 30) + "\t" + appendBlanks("결    과", 21) + " \t" + "참    고    치";
						data[35] += getDivide() + "\r\n" + appendBlanks(inspectName[i], 30) + "\t" + appendBlanks(result[i], 21) + "\t"
								+ getReferenceValueAge(remark);
						data[34] = ""; // 문자결과
				...
				//부산특영 거래처 이미지 파일명 추가
				if((hosCode[i].trim().equals("27443")||hosCode[i].trim().equals("27296")||hosCode[i].trim().equals("27430")||hosCode[i].trim().equals("27773"))&&data[4].trim().length()>0)
				{
					data[19] = data[4]+".jpg";
				}
				...
				if (hosCode[i].trim().equals("28668") && (!inspectCode[i].trim().equals("41052")&&!inspectCode[i].trim().equals("41054")&&!inspectCode[i].trim().equals("41114")&&!inspectCode[i].trim().equals("00045")&&!inspectCode[i].trim().equals("21565")&&!inspectCode[i].trim().equals("41092")
						&&!inspectCode[i].trim().equals("41097")&&!inspectCode[i].trim().equals("41024")&&!inspectCode[i].trim().equals("21430")&&!inspectCode[i].trim().equals("41110"))) { // 단독
					data[39] = "";
				}
