...
public class DownloadHanyang extends ResultDownload {
	boolean isDebug = false;

	boolean isData = false;

	public DownloadHanyang() {
		isDebug = false;
	}

	public DownloadHanyang(String id, String fdat, String tdat, Boolean isRewrite) {
		isDebug = false;
		setId(id);
		setfDat(fdat);
		settDat(tdat);
					...
					if (inspectCode[i].trim().length() == 7 && (hosCode[i].toString().trim().equals("26070")||hosCode[i].toString().trim().equals("15710") && (inspectCode[i].trim().substring(0, 5).equals( "71251")
								...
								&& curNo.equals(rcvNo[i].trim());) 
						{
							if(!isMastDuplPrint(inspectCode[i])){
								data[4] += "@^"+ appendBlanks(inspectName[i], 30)+ "   "+ appendBlanks(result[i], 21)+ "   "+ getReference(new String[] { inspectCode[i],	lang[i], history[i], sex[i],age[i] }).trim();
							}
...
//					o_writer = new OutputStreamWriter(f_outStream);            // 인코딩 수정전
				...
				//의뢰일자(8) + 등록번호(8) + 검체번호(10) + 성명(10) + 위탁코드(10) + 병원검사코드(10) + 연속검사구분(2) + 결과(50)
				text.append(getByteString(fill(rcvDate[i],8),0,8)+ getByteString(fill(chartNo[i],8),0,8)+ getByteString(fill(specNo[i],10),0,10) + getByteString(fill(patName[i],10),0,10) +getByteString(fill(inspectCode[i].trim(),10),0,10)+getByteString(fill(data[3],10),0,10)+ "00"+result_str+ "\r\n");
	...
	public static void main(String[] args){
		String test = "123가나다";
		System.out.println(getByteString(test,0,7));
	}
