...
public class DownloadCityhos extends ResultDownload {
	...
	public DownloadCityhos() {
		// isDebug = false;
	}

	public DownloadCityhos(String id, String fdat, String tdat, Boolean isRewrite) {
		setId(id);
		setfDat(fdat);
		settDat(tdat);
...
//				}
				...
				if (resultType[i].trim().equals("C")) {
					...
					remark[3] = sex[i];
					remark[4] = age[i];
					data[6] = getReferenceValue(remark);	//참고치


					try {
						
							if (inspectCode[i].trim().length() == 7 && (hosCode[i].toString().equals("31574"))) {
								data[3] = appendBlanks("검  사  명 ", 30)  + appendBlanks("결    과", 21) + "참    고    치";
								data[3] += "@^" + appendBlanks(inspectName[i], 30) + "\t" + appendBlanks(result[i], 21) + "\t" + replaceCrLf(getReferenceValue(new String[] {inspectCode[i], lang[i], history[i], sex[i]}).trim());
								curDate = rcvDate[i];
								curNo = rcvNo[i];
								for ( thisTimeCode = inspectCode[i++].trim().substring(0, 5); thisTimeCode.equals(inspectCode[i].trim().substring(0, 5)) && curDate.equals(rcvDate[i].trim()) && curNo.equals(rcvNo[i].trim());) {
										data[3] += "@^" + appendBlanks(inspectName[i], 30) + "\t"  + appendBlanks(result[i], 21)+ "\t" + replaceCrLf(getReferenceValue(new String[] {inspectCode[i], lang[i], history[i], sex[i]}).trim());
//										System.out.println(getReferenceValue(new String[] {inspectCode[i], lang[i], history[i], sex[i]}).trim());
									if (++i == cnt)
										break;
								}
								i--;
...
//						}
			...
			//			data[8] = data[3].trim();
				...
				if ((hosCode[i].toString().equals("31574"))
...
//			      o_writer = new OutputStreamWriter(f_outStream);            // 인코딩 수정전
				...
				// 차트번호 . 검체번호 , 검사코드 ,검사결과
				text.append(data[4] + data[0] + data[1] + deli + data[5] + deli + data[2] + deli + data[3] + "\r\n");
