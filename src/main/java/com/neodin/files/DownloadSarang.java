 ...
 */
public class DownloadSarang extends ResultDownload {
	boolean debug = false;
	public DownloadSarang() {
		initialize();
	}
	...
	public DownloadSarang(String id, String fdat, String tdat, Boolean isRewrite) {
		setId(id);
		setfDat(fdat);
		settDat(tdat);
	...
	public DownloadSarang(String id, String fdat, String tdat, Boolean isRewrite,String hakCd) {
		setId(id);
		setfDat(fdat);
		settDat(tdat);
		setIsRewrite(isRewrite.booleanValue());
	...
	public void makeDownloadFile() {
		{
			...
			try {
				...
				ArraryResult = (new String[] { "CLINIC_NO", "ORDER_NO", "ORDER_SEQ",
						"USER_CD", "USER_NM", "VAL1", "VAL2", "VAL3", "ORDER_DT",
						"SEQ", "PATIENT_NM", "PATIENT_CD", "BIRTH_DT", "SEX"
						});
				if (!debug) {
