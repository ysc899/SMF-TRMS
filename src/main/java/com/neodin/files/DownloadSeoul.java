...
public class DownloadSeoul extends ResultDownload {
	boolean isDebug = false;

	boolean isData = false;

	public DownloadSeoul() {
		isDebug = false;
		initialize();
	}

	public DownloadSeoul(String id, String fdat, String tdat, Boolean isRewrite) {
		isDebug = false;
		setId(id);
		setfDat(fdat);
		settDat(tdat);
	...
	public void makeDownloadFile() {
		row = 1;
		try {
			workbook = jxl.Workbook.createWorkbook(new File(savedir
					+ makeOutFile()));
