...
--  Generated on:              23/01/05 09:24:50 
--  Relational Database:       NEODIN 
--  Standards Option:          Db2 for i 
--DROP SPECIFIC PROCEDURE IF EXISTS WEBOBJLIB.MWGRPRPT ; 
--SET PATH "QSYS","QSYS2","SYSPROC","SYSIBMADM","NEO018" ; 
CREATE OR REPLACE PROCEDURE WEBOBJLIB.MWGRPRPT ( 
  IN I_PARAM VARCHAR(15000) , 
  IN I_UID VARCHAR(12) , 
  IN I_IP VARCHAR(30) , 
  IN I_REPORT_LANGUAGE VARCHAR(1000) ) 
  DYNAMIC RESULT SETS 1 
  LANGUAGE SQL 
  SPECIFIC WEBOBJLIB.MWGRPRPT 
  NOT DETERMINISTIC 
  MODIFIES SQL DATA 
  ...
  BEGIN 
	...
	DECLARE CUR1 CURSOR WITH RETURN FOR 
		...
		SELECT DISTINCT 
			...
			, CASE WHEN TRIM ( I_REPORT_LANGUAGE ) = 'eng' THEN Y . S018RFE  -- 리포트 템플릿 파일명(영문) 
				ELSE 
					CASE WHEN PDF_CNT > 0 THEN '78-STI-TMJ-01' ELSE COALESCE ( Y . S018RFN , 'GENERAL-01' ) END  -- 리포트 템플릿 파일명 (국문) 
				END AS S018RFN 
			, F100JKN  --진료과목 
			, F100DRC  --진료의사 
			, F111GDT  --검체최취일 
			...
			WHEN TRIM ( CU_SE ) = 'SE' THEN CONCAT ( ( SELECT 
				...
				, IFNULL ( ( SELECT 
						LISTAGG ( CASE WHEN TRIM ( S012NCL ) = 'CHART' THEN TRIM ( F100CHN )  --차트번호 
									...
									WHEN TRIM ( S012NCL ) = 'TCD' THEN ( CASE WHEN ( IFNULL ( TRIM ( F100HOS ) , '' ) = '27466' AND IFNULL ( TRIM ( F600GCD ) , '' ) = '51023' ) THEN ( SELECT TRIM ( FHCDTCD ) FROM MCLISDLIB . "MCHCDH@" M 
																																												WHERE 1 = 1 
					...
					, CONCAT ( CONCAT ( CONCAT ( CONCAT ( F100CHN , '_' ) , CONCAT ( F100NAM , '_' ) ) , CONCAT ( TRIM ( ( CASE WHEN IFNULL ( TRIM ( FHCDTCD_1 ) , '' ) <> '' THEN FHCDTCD_1 
																							...
																							, CONCAT ( ( CASE WHEN IFNULL ( HOS_GCD01 , '' ) <> '' THEN HOS_GCD01 
																										...
																										WHEN IFNULL ( HOS_GCD03 , '' ) <> '' THEN HOS_GCD03 
																											WHEN IFNULL ( HOS_GCD04 , '' ) <> '' THEN HOS_GCD04 
																											WHEN IFNULL ( HOS_GCD05 , '' ) <> '' THEN HOS_GCD05 
																											ELSE TRIM ( F600GCD ) END ) , '_01' ) ) ) AS FILE_NM  --파일명 구분 
			...
			FROM ( 
				SELECT 
					...
					, CASE WHEN TRIM ( REPLACE ( A . F600CHR , ' ' , '' ) ) LIKE '%별지%' 
					 ...
					 --, TRIM(REPLACE(A.F600CHR,' ','' )) AS F600CHR  --별지참조 
					 -- 파일명에 사용할 검체코드(샘플번호) 파일명을 위한 ETC2(파일 생성시 오루나는 문자 제거) 
...
, ( SELECT TRIM ( F033TCD ) 
				...
				, ( SELECT 
						...
						AND FHCDDAT = F600DAT 
						AND FHCDJNO = F600JNO 
						AND FHCDGCD = CONCAT ( F600GCD , '01' )  -- 2022.09.01 : 담당자요청으로 
--조건 원복 
						 --AND FHCDGCD = ( CASE WHEN F600HOS = '27466' AND F600GCD = '51023' THEN 
--'68028' ELSE CONCAT ( F600GCD , '01' ) END ) 
				...
				AND ( A . F600COR , A . F600DAT , A . F600JNO , A . F600GCD ) IN (	 
					SELECT 
						SUBSTRING ( DWARF , 1 , 3 ) AS COR 
						, INTEGER ( SUBSTRING ( DWARF , 4 , 8 ) ) AS DAT 
						 --, INTEGER ( SUBSTRING ( DWARF , 4 + 8 , 7 ) ) AS JNO  --IDR 노영곤 파라메터 길이변경(5->7) 
						 --, SUBSTRING ( DWARF , 4 + 8 + 7 , 5 ) AS GCD  --IDR 노영곤 파라메터 길이변경(5->7) 
, ( CASE WHEN LENGTH ( DWARF ) < 23 THEN INTEGER ( SUBSTRING ( DWARF , 4 + 8 , 5 ) ) 
							ELSE INTEGER ( SUBSTRING ( DWARF , 4 + 8 , 7 ) ) END ) AS JNO  -- cjw 재수정 
, ( CASE WHEN LENGTH ( DWARF ) < 23 THEN SUBSTRING ( DWARF , 4 + 8 + 5 , 5 ) 
							ELSE SUBSTRING ( DWARF , 4 + 8 + 7 , 5 ) END ) AS GCD  -- cjw 재수정 
					FROM ( 
						SELECT 
								SUBSTR ( NAME , 1 , INSTR ( NAME , '|' ) - 1 ) DWARF 
						FROM ( 
						SELECT N , SUBSTR ( PRAM , 1 + INSTR ( PRAM , '|' , 1 , N ) ) NAME 
						FROM ( 
						SELECT ROW_NUMBER ( ) OVER ( ) AS N , L . PRAM 
						FROM ( SELECT I_PARAM PRAM 
						FROM "SYSIBM" . SYSDUMMY1 ) L 
						CONNECT BY LEVEL < 
						( LENGTH ( L . PRAM ) 
						- LENGTH ( REPLACE ( L . PRAM , '|' , '' ) ) 
						) 
						) AS B 
						) C 
						) PARM_LIST 
		 ...
		 --WHERE (CASE WHEN TRIM(I_REPORT_LANGUAGE) = 'eng' THEN  Y.S018RFE ELSE Y.S018RFN END) <> '' 
