...
--  Generated on:              23/01/05 09:31:05 
--  Relational Database:       NEODIN 
--  Standards Option:          Db2 for i 
--DROP SPECIFIC PROCEDURE IF EXISTS WEBOBJLIB.MWIMGGRPRPT2 ; 
--SET PATH "QSYS","QSYS2","SYSPROC","SYSIBMADM","NEO018" ; 
CREATE OR REPLACE PROCEDURE WEBOBJLIB.MWIMGGRPRPT2 ( 
  IN I_COR VARCHAR(3) , 
  IN I_HOS VARCHAR(10) , 
  ...
  LANGUAGE SQL 
  SPECIFIC WEBOBJLIB.MWIMGGRPRPT2 
  NOT DETERMINISTIC 
  MODIFIES SQL DATA 
			...
			WHEN TRIM ( CU_SE ) = 'SE' THEN CONCAT ( ( SELECT 
				...
				, IFNULL ( ( SELECT 
						LISTAGG ( CASE WHEN TRIM ( S012NCL ) = 'CHART' THEN TRIM ( F100CHN )  --차트번호 
									...
									WHEN TRIM ( S012NCL ) = 'TCD' THEN ( CASE WHEN IFNULL ( TRIM ( F100ETC ) , '' ) = '' THEN '검체번호' ELSE TRIM ( F100ETC ) END )  -- 검체코드 
									WHEN TRIM ( S012NCL ) = 'GCD' THEN TRIM ( F600GCD )  -- 검사코드 
									WHEN TRIM ( S012NCL ) = 'HOS_GCD' THEN ( CASE WHEN IFNULL ( HOS_GCD01 , '' ) <> '' THEN HOS_GCD01  -- 병원검사코드 
					...
					, CONCAT ( CONCAT ( CONCAT ( CONCAT ( F100CHN , '_' ) , CONCAT ( F100NAM , '_' ) ) , CONCAT ( ( CASE WHEN IFNULL ( TRIM ( F100ETC ) , '' ) = '' THEN '검체번호' ELSE F100ETC END ) , '_' ) ) , CONCAT ( ( CASE WHEN IFNULL ( HOS_GCD01 , '' ) <> '' THEN HOS_GCD01 
																																																	WHEN IFNULL ( HOS_GCD02 , '' ) <> '' THEN HOS_GCD02 
																																																WHEN IFNULL ( HOS_GCD03 , '' ) <> '' THEN HOS_GCD03 
																																																	WHEN IFNULL ( HOS_GCD04 , '' ) <> '' THEN HOS_GCD04 
																																																	WHEN IFNULL ( HOS_GCD05 , '' ) <> '' THEN HOS_GCD05 
